...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.num_classes = torch.tensor(config.num_labels,dtype=torch.float)
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if not self.training:
                ...
                highway_entropy = entropy(highway_logits,self.num_classes)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        if not self.training:
            original_entropy = entropy(logits,class_num=torch.tensor(self.num_labels,dtype=torch.float))
            student_entropy = []
            student_logits_all = []
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = MSELoss()
                teacher_loss = loss_fct(logits.view(-1), labels.view(-1))
            else:
                loss_fct = CrossEntropyLoss()
                teacher_loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            ...
            if train_highway:
                ...
                for highway_exit in outputs[-1]:
                    student_logits = highway_exit[0]
                    if not self.training:
                        student_logits_all.append(student_logits)
                        student_entropy.append(highway_exit[2])
            
                    #student_loss = torch.softmax(logits,dim = -1) * (torch.log_softmax(logits,dim = -1) - torch.log_softmax(student_logits,dim = -1))
                    #student_loss = torch.sum(student_loss,dim = -1)
                    #student_loss = torch.mean(student_loss,dim = -1)
                    target_loss = loss_fct(student_logits,labels)
                    student_losses.append(target_loss)
                outputs = (sum(student_losses[:-1]) + teacher_loss,) + outputs
                    # exclude the final highway, of course
            else:
                outputs = (teacher_loss,) + outputs
        if not self.training:
            outputs = outputs + ((original_entropy, student_entropy), exit_layer)
