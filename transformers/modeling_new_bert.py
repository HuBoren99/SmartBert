...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        query_layer = nn.Softmax(dim=-1)(query_layer)
        key_layer = nn.Softmax(dim=-2)(key_layer)

        kv = torch.matmul(key_layer.transpose(-1,-2),value_layer)
        context_layer = torch.matmul(query_layer,kv)/self.attention_head_size
        ...
        outputs = (context_layer, kv, save_layer) if self.output_attentions else (context_layer,)
        return outputs
