...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        save_layer = context_layer
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)

        outputs = (context_layer, attention_probs, save_layer) if self.output_attentions else (context_layer,)
        return outputs
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        if self.training:
            ...
            gate_output = gate_output + (torch.ge(gate_output,0.5).float() - gate_output).detach()
            self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask)
            attention_output = self_attention_outputs[0] * (1 - gate_output[:,:,None]) + hidden_states * gate_output[:,:,None]
